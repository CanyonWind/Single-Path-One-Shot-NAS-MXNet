...
"""Operation definition for seperated activation.
Attributes:
  output_shape: list, of length 4. Shape of the output feature map
  activation_name: str, type of activation applied to the output.
"""

Activation = collections.namedtuple(
    'Activation', ['output_shape', 'activation_name'])
...
def get_flops_per_activation(activation):
  ...
  if activation == 'relu':
    ...
    return 2, 1
  elif activation == '_plus_scalar':  # Hard-swish: x * (F.clip(x + 3, 0, 6) / 6.)
    return 0, 1
  elif activation == 'clip':
    return 1, 0
  elif activation == '_div_scalar':
    return 1, 0
  elif activation == 'elemwise_mul':
    return 1, 0
  else:
...
def get_info(op):
  ...
  if isinstance(op, (DepthWiseConv2D, Conv2D)):
    ...
    in_channels, out_channels = op.output_shape[1], op.output_shape[1]
    input_size = op.output_shape[2]
  else:
    raise ValueError('Encountered unknown operation %s.' % str(op))
